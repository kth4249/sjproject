...
public class BoardDao {
	...
	public BoardDao() throws Exception{
		String fileName =
				BoardDao.class
				.getResource("/com/kh/sjproject/sql/board/board-query.properties").getPath();
		...
		try {
			...
			while(rset.next()) {
				board = new Board(rset.getInt("BOARD_NO"),
						rset.getString("BOARD_TITLE"), rset.getInt("BOARD_COUNT"),
						rset.getDate("BOARD_MODIFY_DT"), rset.getString("MEMBER_ID"), 
	...
	public List<Attachment> selectFiles(Connection conn, int boardNo) throws Exception{
		...
		try {
			...
			while(rset.next()) {
				file = new Attachment(rset.getInt(1), rset.getInt(2),
	...
	public List<Reply> selectReplyList(Connection conn, int boardId) throws Exception{
		...
		try {
			...
			while(rset.next()) {
				reply = new Reply(rset.getInt("REPLY_NO"), rset.getString("REPLY_CONTENT"),
						rset.getInt("BOARD_ID"), rset.getString("MEMBER_ID"), rset.getTimestamp("REPLY_MODIFY_DT"));
