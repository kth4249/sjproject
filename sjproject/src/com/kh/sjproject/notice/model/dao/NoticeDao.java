...
public class NoticeDao {
	...
	public NoticeDao() throws Exception{
		String fileName =
				NoticeDao.class
				.getResource("/com/kh/sjproject/sql/notice/notice-query.properties").getPath();
	...
	public Notice selectNotice(Connection conn, int no) throws Exception{
		...
		try {
			...
			if(rset.next()) {
				notice = new Notice(no,rset.getString("NOTICE_TITLE"),
						rset.getString("NOTICE_CONTENT"),rset.getString("NOTICE_WRITER"),
						rset.getInt("NOTICE_COUNT"), rset.getDate("NOTICE_MODIFY_DT"));
			}
